...
async function seed() {
  ...
  services.forEach(s => batch.set(doc(collection(db, 'services'), s.slug), s));
  ...
  const oldPortfolio = await getDocs(collection(db, 'portfolio'));
  oldPortfolio.forEach(d => batch.delete(d.ref));
  portfolio.forEach(p => batch.set(doc(collection(db, 'portfolio')), p));
  ...
  const oldRefs = await getDocs(collection(db, 'references_logos'));
  oldRefs.forEach(d => batch.delete(d.ref));
  refs.forEach(r => batch.set(doc(collection(db, 'references_logos')), r));
  ...
  const oldPackages = await getDocs(collection(db, 'packages'));
  oldPackages.forEach(d => batch.delete(d.ref));
  packages.forEach(p => batch.set(doc(collection(db, 'packages')), p));
  ...
  Object.entries(settings).forEach(([key, value]) => {
    batch.set(doc(collection(db, 'settings'), key), { value: String(value) });
  });

  await batch.commit();
  console.log("SEEDED SUCCESSFULLY VIA CLIENT SDK!");
  process.exit(0);
}
